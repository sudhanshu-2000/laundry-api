...
app.use(
  cors({ origin: ['https://admin.thelaundrywalas.com','http://admin.thelaundrywalas.com','http://localhost:4200'] })
);
            ...
            if (result[0] != null) {
              ...
              if (match) {
                jwt.sign(
                  ...
                  process.env.SECRET_KEY_SUPERADMIN, { expiresIn: '60d' },
                  (err, token) => {
                    if (err) throw err;
                    else
          ...
          (err, result) => {
            ...
            if (result[0] != null) {
              ...
              if (match) {
                jwt.sign(
                  { username: result[0].username },
                  process.env.SECRET_KEY_ADMIN, { expiresIn: '60d' },
                  (err, token) => {
                    if (err) throw err;
                    else
                      con.query("UPDATE `login` SET `is_active`='Y' WHERE `username` = ?", [req.body.username])
...
app.post("/update-bill-status", verifytoken, (req, res) => {
  if (req.body.status == 'C') {
    ...
    con.query(
      "select `bill_genrate` as bg from `order` where `order_no`=?",
      [req.body.order_no],
      (err, result) => {
        if (err) throw err;
        if (result[0].bg == 'C') {
          ...
          con.query(
            "UPDATE `order` SET `status`=? WHERE `order_no`= ?",
